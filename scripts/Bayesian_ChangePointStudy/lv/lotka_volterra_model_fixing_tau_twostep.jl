...
i = local_machine ? 15 : parse(Int, ARGS[1])
...
#read the esimtated change points from multiple R functions
csv_reader = CSV.read(
    "$(@__DIR__)/lv_change_point_res.csv"
, DataFrame)

FIX_TAU = csv_reader[i, changepoint_method] #3:segmented; 4:Rbeast; 5:mcp; 6:strucchange
...
priors = (
    ...
    p_invar=(δ=PH.fixed(0.4),),
    event_times=(τ₁=PH.fixed(FIX_TAU / 100),),
    scaling=(
        SCALING_p_var=(SCALING_α=PH.fixed(ones(2)), SCALING_β=PH.fixed(1e-2 * ones(2))),
        SCALING_p_invar=PH.fixed([1.0]),
        SCALING_event_times=PH.fixed(1e1 * ones(1)),
...
##====================
# compare the true and guess solutions
##====================
...
true_params_flatten, _ = PH.value_flatten(orig_pt_sample);
p_var_guess, p_invar_guess, event_times_guess = build_ps(guess_init_pt; return_array=false)
cp_guess = ChangePointParameters(
    u0_expanded, p_var_guess, p_invar_guess, event_times_guess, set_of_p_var, Dict()
)
sol_guess = solve_event_times(
    cp_guess, ts, prob_embed_sample, alg, sensealg; solvesettings...
)
plt_guess = single_plot_solutions(
    sol_guess;
    loss_function_kwargs=loss_function_kwargs,
    event_times_true=event_times_sample,
    name="guess",
    loss=round(loss_function(guess_params_flatten); digits=3),
)
...
new_chain_ = scale_chain(new_chain, unflatten, [5, 6]) # 5, 6 are the indices of δ and τ
summarize(new_chain)
...
# estim_pt = unflatten(estim_params_end.minimizer)
...
post_df_ = DataFrame(new_chain_)[:, new_names]
param_dict = DataFrame(param_dict)
@unpack data_true, mean_idxs, var_idxs = loss_function_kwargs
data_true = reduce(vcat, transpose.(data_true))'
